...
const REHYPE_ICON_DEFAULT_REPLACES = {
  js: "JavaScriptIcon",
  jsx: "JavaScriptIcon",
  ts: "TypeScriptIcon",
  tsx: "TypeScriptIcon",
...
const rehypeIcon = (replaces = REHYPE_ICON_DEFAULT_REPLACES) => (ast) => {
  const imports = ast.children.filter(
    (node) => isMdxJsEsm(node) && isImportDeclaration(node) && isImportFrom(node)
  );
