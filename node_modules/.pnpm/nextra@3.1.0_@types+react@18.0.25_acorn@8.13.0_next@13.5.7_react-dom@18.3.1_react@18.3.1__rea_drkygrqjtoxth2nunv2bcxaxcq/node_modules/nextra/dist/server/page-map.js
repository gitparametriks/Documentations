...
function cleanFileName(name) {
  return path.relative(PAGES_DIR, name).replace(/\.([jt]sx?|mdx?)$/, "").replaceAll(/[\W_]+/g, "_").replace(/^_/, "").replace(/^\d/, (match) => `_${match}`);
}
async function collectFiles({
  ...
  const promises = files.sort((a, b) => a.name.localeCompare(b.name)).map(async (f) => {
    ...
    if (isDirectory) {
      ...
      const { pageMap: children, hasDynamicPage: hasDynamicPage2 } = await collectFiles({
        dir: filePath,
        route: fileRoute,
        imports,
        dynamicMetaImports,
      ...
      if (!children.length && !hasDynamicPage2) return;
      return {
        name: f.name,
        route: fileRoute,
        children
    ...
    return limit(async () => {
      if (MARKDOWN_EXTENSION_REGEX.test(ext)) {
        ...
        if (!data.title) {
          data.sidebarTitle || (data.sidebarTitle = pageTitleFromFilename(name));
        }
        return {
          name,
          route: fileRoute,
      ...
      if (fileName === "_meta.json") {
        throw new Error(
          `Support of "_meta.json" was removed, use "_meta.{js,jsx,ts,tsx}" instead. Refactor following file "${path.relative(CWD, filePath)}".`
        );
      }
      if (isMetaJs) {
...
function convertPageMapToAst(pageMap) {
  const elements = pageMap.map((item) => {
    ...
    return createAstObject({
      data: typeof item.data === "string" ? { type: "Identifier", name: item.data } : valueToEstree(item.data)
    });
  });
  return { type: "ArrayExpression", elements };
}
async function collectPageMap({
  dir,
  route = "/",
  locale = "",
  transformPageMap
  ...
  const metaImportsAST = imports.sort((a, b) => a.filePath.localeCompare(b.filePath)).map(({ filePath, importName }) => ({
      ...
      declarations: [
        {
          ...
          init: {
            ...
            properties: dynamicMetaImports.sort((a, b) => a.route.localeCompare(b.route)).map(({ importName, route: route2 }) => ({
              ...DEFAULT_PROPERTY_PROPS,
              key: { type: "Literal", value: route2 },
